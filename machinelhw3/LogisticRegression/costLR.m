...
%    [cost, grad, hess] = costLR(A, B, C); % hessian is optional
    ...
    %% BEGIN SOLUTION --- toc - tic <= 1ms
  
    wtfeat = X * theta; % weighted features. saved to avoid repetitive computation.
    cost = sumsq(theta) * 0.5 * lambda - y' * wtfeat + sum(logsumexp(prepad(wtfeat, 2, 0, 2)));
    grad = lambda * theta - ((y - sigmoid(wtfeat))' * X)'; 
